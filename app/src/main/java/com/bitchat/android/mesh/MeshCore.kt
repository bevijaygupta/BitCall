...
import com.bitchat.android.calling.tier1.MeshCallSignaling
...
class MeshCore(
    ...
    private val callSignaling = MeshCallSignaling(
        sendNoisePayload = { peerID, payload -> sendNoisePayloadToPeer(payload, peerID) },
        onSignal = { peerID, signal ->
            callSignalHandler?.invoke(peerID, signal)
            delegate?.didReceiveCallSignal(peerID, signal)
        }
    )
    ...
    private val pendingCallSignals = ConcurrentHashMap<String, MutableList<CallSignalingPacket>>()
    ...
    var callSignalHandler: ((peerID: String, signal: CallSignalingPacket) -> Unit)? = null
    ...
    init {
        ...
        messageHandler.callSignalHandler = callSignaling::receive
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            override fun onKeyExchangeCompleted(
                ...
                scope.launch {
                    ...
                    flushPendingCallSignals(peerID)
                }
            }
    ...
    fun sendCallSignal(recipientPeerID: String, signal: CallSignalingPacket) {
        if (recipientPeerID.isBlank()) return
        scope.launch {
            if (!encryptionService.hasEstablishedSession(recipientPeerID)) {
                pendingCallSignals.compute(recipientPeerID) { _, queued ->
                    (queued ?: mutableListOf()).also { it += signal }
                }
                initiateNoiseHandshake(recipientPeerID)
                return@launch
            }
            callSignaling.send(recipientPeerID, signal)
        }
    }

    private fun flushPendingCallSignals(peerID: String) {
        if (!encryptionService.hasEstablishedSession(peerID)) return
        val queued = pendingCallSignals.remove(peerID) ?: return
        queued.forEach { signal -> callSignaling.send(peerID, signal) }
    }
