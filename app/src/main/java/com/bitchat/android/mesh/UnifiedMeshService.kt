...
class UnifiedMeshService(
    ...
    override fun sendCallSignal(recipientPeerID: String, signal: CallSignalingPacket) {
        when {
            isBleReady(recipientPeerID) -> bluetooth.sendCallSignal(recipientPeerID, signal)
            isWifiReady(recipientPeerID) -> wifiService()?.sendCallSignal(recipientPeerID, signal)
            isBleConnected(recipientPeerID) || (isBleEnabled() && !isWifiConnected(recipientPeerID)) ->
                bluetooth.sendCallSignal(recipientPeerID, signal)
            else -> wifiService()?.sendCallSignal(recipientPeerID, signal)
        }
    }

    override fun getCallAudioSocket(peerID: String): SyncedSocket? =
        when {
            isWifiReady(peerID) -> wifiService()?.getCallAudioSocket(peerID)
            else -> null
        }

    override fun registerCallAudioSession(peerID: String, session: WifiAwareAudioSession) {
        wifiService()?.registerCallAudioSession(peerID, session)
    }
